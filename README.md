...
The player site is **Netlify**. The API, Postgres, and Redis are **Render**.

1. Push this repo to GitHub (`https://github.com/Doriel1121/solo-leveling`).
2. Apply the blueprint: [Create Render Blueprint](https://dashboard.render.com/blueprint/new?repo=https://github.com/Doriel1121/solo-leveling). Fill `GEMINI_API_KEY`, `COHERE_API_KEY`, and a temporary `CORS_ORIGIN` (you can use `http://localhost:3000` until the Netlify URL exists).
...
The API binds `0.0.0.0:$PORT`, holds no state in memory, and runs migrations on boot. Render's free web tier sleeps after 15 minutes and would drop in-flight SSE streams — the blueprint uses a starter plan for that reason.
